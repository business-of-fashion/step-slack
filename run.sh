...
if [ "$WERCKER_RESULT" = "failed" ]; then
  ...
  export COLOR="danger"
  if [ -n "$WERCKER_SLACK_NOTIFIER_FAILED_MESSAGE" ]; then
      export MESSAGE="$MESSAGE\n$WERCKER_SLACK_NOTIFIER_FAILED_MESSAGE"
      export FALLBACK="$FALLBACK\n$WERCKER_SLACK_NOTIFIER_FAILED_MESSAGE"
  fi
else
  if [ -n "$WERCKER_SLACK_NOTIFIER_PASSED_MESSAGE" ]; then
      export MESSAGE="$MESSAGE\n$WERCKER_SLACK_NOTIFIER_PASSED_MESSAGE"
      export FALLBACK="$FALLBACK\n$WERCKER_SLACK_NOTIFIER_PASSED_MESSAGE"
  fi
fi
